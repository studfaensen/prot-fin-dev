...
if ((length(args) - 1) %% 2 != 0 | (length(args) - 1) < 2) {
    message("Usage: Rscript raincloud_plot.R (<value-title> <values>)... <out-file.png>\n")
    message("Set title with environment variable TITLE and label of x axis with X_LABEL\n")
    message("Example:")
    message("X_LABEL=Lengths TITLE=\"Length Distribution\" Rscript raincloud_plot.R \"Case 1\" \"1,2,3\" \"Case 2\" \"2,2,2\" out-file.png")
    q()
}
